...
\begin{equation}\label{eq_c_ij&eq_c_k1}
...
\begin{equation}\label{eq_m_n_req}
M_n = \frac{(2n+1)M_{n-1} + 3(n-1)M_{n-2}}{n+2},\quad n \ge 2.
\end{equation}
Funkcja tworząca:
\begin{equation}\label{eq_m_n_func}
...
\begin{equation}\label{eq_m_2/3/4}
\begin{aligned}
...
\begin{equation}\label{eq_S_ji}
S_{i,j}(\pi) = (\pi_0,\ldots,\pi_{i-1},\; \pi_j,\; \pi_{i+1},\ldots,\pi_{j-1},\; \pi_i,\; \pi_{j+1},\ldots,\pi_{n-1}).
\end{equation}
Ta operacja zamienia wyłącznie elementy na pozycjach $i$ i $j$, zachowując kolejność wszystkich elementów pośrednich $\pi_{i+1},\ldots,\pi_{j-1}$. Lokalnie cały segment $[\pi_i,\pi_{i+1},\ldots,\pi_{j-1},\pi_j]$ przekształca się w $[\pi_j,\pi_{i+1},\ldots,\pi_{j-1},\pi_i]$.
...
\begin{figure}[ht] \label{Przykład_dopuszczalnego_ruchu_złożonego}
  \centering
...
\section[Algorytm wyboru optymalnego \ldots]{Algorytm wyboru optymalnego ruchu złożonego}
...
\begin{equation}\label{new_perm}
\begin{aligned}
\relax[\pi_j, \pi_{i+1}, \ldots, \pi_{j-1}, \pi_i],
\end{aligned}
\end{equation}
...
\paragraph{Rekurencja programowania dynamicznego.} Definiujemy funkcję\linebreak $\mathrm{dp}[L][R]$ jako \emph{minimalną sumę delt} osiągalną przez wybór dopuszczalnego zbioru par w przedziale indeksów $[L, R]$. Warunki brzegowe:
\begin{equation}\label{eq_dp_LR}
...
Rekurencja:
\begin{equation}\label{eq_dp_LR_req}\scriptstyle
...
\begin{equation}\label{eq_choise_LR}
\mathrm{choice}[L][R] = 
\begin{cases}
...
\begin{equation}\label{eq_P_matrix}
P = \begin{bmatrix}
3 & 2 & 5 & 4 & 3 \\
2 & 4 & 3 & 2 & 5 \\
4 & 3 & 2 & 5 & 4
...
\begin{equation}\label{eq_cmachnies_jobs}
\begin{array}{c|ccccc}
...
\begin{equation}\label{eq_F_matrix}
F = \begin{bmatrix}
0 & 0 & 0 \\
3 & 5 & 9 \\
5 & 9 & 12 \\
...
\begin{equation}\label{eq_delat_matrix}
\Delta = \begin{bmatrix}
 & 0 & +2 & -1 & 0 \\
 & \cdot & +1 & +3 & -2 \\
 & \cdot & \cdot & +2 & -3 \\
...
\begin{equation}\label{eq_pi_prim}
\pi' = (0, 1, 4, 3, 2).
\end{equation}

Obliczamy nowy makespan:
\begin{equation}\label{eq_C_mar_pi_prim}
C_{\max}(\pi') = 25 - 3 = 22.
\end{equation}

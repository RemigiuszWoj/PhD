...
standard use today~\cite{taillard1993}. Among constructive heuristics,
the method of Nawaz, Enscore, and Ham is still regarded as the strongest
of its kind and is the usual starting point for local
search~\cite{neh1983}. Ruiz and Maroto compared a large collection of
heuristics and metaheuristics for the makespan objective and confirmed
that iterated local search and simulated annealing are among the most
effective approaches~\cite{ruizmaroto2005}. The power of such methods
depends on how large a neighborhood they can search at each step. Ahuja,
Ergun, Orlin, and Punnen surveyed very large-scale neighborhood search,
in which a neighborhood of exponential size is explored in polynomial
time~\cite{ahuja2002}. Congram, Potts, and van de Velde introduced
dynasearch, which uses dynamic programming to apply several independent
interchange moves in a single step and gives its name to the Dynasearch
neighborhood studied here~\cite{congram2002}.
